...
st.set_page_config(page_title="Envio por E-mail | Análise de Churn", page_icon="📧", layout="wide")
...
opcao = st.radio(
    "Tipo de conteúdo",
    ["📊 Gráfico estático (imagem em anexo)", "📝 Relatório de insights (texto)"],
    horizontal=True,
)

st.markdown("### 2. Detalhes do envio")

destinatario = st.text_input("E-mail de destino", placeholder="seuemail@exemplo.com")
...
if opcao.startswith("📊"):
    grafico_escolhido = st.selectbox("Selecione o gráfico", list(GRAFICOS_DISPONIVEIS.keys()))
    ...
    with st.expander("👀 Pré-visualizar relatório de insights", expanded=True):
        st.text(relatorio)
    corpo_extra = "Segue em anexo o relatório de insights e análises sobre o churn de clientes."
...
if st.button("📨 Enviar e-mail", type="primary", use_container_width=True):
    if not destinatario:
        ...
        try:
            ...
            st.info(
                "Se estiver rodando localmente, configure o arquivo "
                "`.streamlit/secrets.toml` com suas credenciais SMTP. "
